  ...
  stages {
    stage('Calculator Website Image build and Release') {
      steps {
        ...
        sh 'docker buildx ls | grep "raspberry-builder" || docker buildx create --name raspberry-builder --platform "linux/arm64,linux/arm/v7,linux/arm/v6" --driver "docker-container"'
